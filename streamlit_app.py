...
# mydb = mysql.connect(host="localhost", user="root", passwd="12345", database='online_railway_db')
mydb = mysql.connect(host="railway.mysql.database.azure.com", user="railwayadmin", passwd="Root@Indian", database='online_railway_db')
sql = mydb.cursor()
...
def show_main_page():
    with mainsection:
        ...
        'Search Trains', 'Book Tickets', 'Cancel Tickets', 'Booking History', 'Edit Your Details', 'Reset Password',
        'Logout'))
        ...
        if page == 'Search Trains':
            ...
            destination = st.selectbox('Destination', options=['---'] + train_df[train_df['Tr From'] == source][
                'Tr To'].unique().tolist())
            ...
            if source != '---' and destination != '---':
                ...
                train_df_filter = train_df_filter.rename(
                    columns={'Tr Number': 'Train Number', 'Tr Name': 'Train Name', 'Tr Dept Time': 'Source Time',
                             'Tr Arr Time': 'Destination Time', 'Tr Total Seats': 'Total Seats',
                             'Tr Avl Seats': 'Available Seats'})
            ...
            if select_train != '---':
                number_of_person = st.number_input('Number of Passengers', min_value=1, max_value=
                train_df_filter[train_df_filter['Tr Name'] == select_train]['Tr Avl Seats'].values[0])
            else:
            ...
            for i in range(number_of_person):
                seat_numbers_generated.append(random.randint(1, train_df_filter[
                    train_df_filter['Tr Name'] == select_train]['Tr Total Seats'].values[0]))
            ...
            if number_of_person != 0:
                total_price = number_of_person * \
                              train_df_filter[train_df_filter['Tr Name'] == select_train]['Tr Price'].values[0]
            ...
            if st.button('Pay and Book'):
                ...
                sql.execute(
                    'INSERT INTO ticket (`Ticket Number`,`Pass ID`,`Tr Number`,`Date`,`Number of Pass`,`Seats Numbers`,`Total Prices`) VALUES (%s,%s,%s,%s,%s,%s,%s)',
                    (int(ticket_number_generated), int(pass_id), train_number, date, number_of_person, seat_numbers,
                     int(total_price)))
                mydb.commit()
            ...
            if ticket_number != '---':
                ...
                merged = merged[['Ticket Number', 'Tr Number', 'Tr Name', 'Tr From', 'Tr To', 'Date', 'Number of Pass',
                                 'Seats Numbers', 'Total Prices']]
                merged = merged.rename(
                    columns={'Tr Number': 'Train Number', 'Tr Name': 'Train Name', 'Tr From': 'Source',
                             'Tr To': 'Destination', 'Date': 'Date of Journey',
                             'Number of Pass': 'Number of Passengers', 'Seats Numbers': 'Seat Numbers',
                ...
                if st.button('Yes'):
                    ...
                    refund_msg = 'You will be refunded the amount of Rs. ' + format(
                        not_journed[not_journed['Ticket Number'] == ticket_number]['Total Prices'].values[0])
                    st.info(refund_msg)
            ...
            ticket_df_join = pd.merge(ticket_df[['Ticket Number', 'Pass ID', 'Tr Number']], train_df,
                                      on='Tr Number').merge(ticket_df.drop(['Tr Number', 'Pass ID'], axis=1),
                                                            on='Ticket Number')
            ticket_df_join = ticket_df_join.rename(
                columns={'Tr Number': 'Train Number', 'Tr Name': 'Train Name', 'Tr Dept Time': 'Source Time',
                         'Tr Arr Time': 'Destination Time', 'Tr Total Seats': 'Total Seats',
                         'Tr Avl Seats': 'Available Seats'})
            # Booking History
            ...
            ticket_df_join = ticket_df_join.rename(
                columns={'Tr Name': 'Train Name', 'Tr Dept Time': 'Source Time', 'Tr Arr Time': 'Destination Time',
                         'Tr Total Seats': 'Total Seats', 'Tr Avl Seats': 'Available Seats'})
            ...
            if st.button('Update'):
                sql.execute(
                    'update passengers set `Pass Name` = %s, `Pass Age` = %s, `Pass Gender` = %s, `Pass Address` = %s, `Pass Contacts` = %s where `Pass ID` = %s',
                    (name, age, gender, address, contact, int(pass_id)))
                mydb.commit()
            ...
            if st.button('Reset'):
                if password == con_password:
                    sql.execute('UPDATE login SET Password = %s WHERE UserName = %s',
                                (password, st.session_state['username']))
                    mydb.commit()
    ...
    with registersection:
        with st.form('register_form'):
            ...
            if st.form_submit_button('Register now'):
                if len(name) > 3 and len(address) > 3 and len(str(contact)) == 10 and len(username) > 3 and len(
                        password) > 3 and password == con_password and agree:
                    ...
                    try:
                        ...
                        if password == con_password and username not in old_user_names and age > 18 and age < 100:
                            ...
                            sql.execute('insert into passengers values (%s,%s,%s,%s,%s,%s)',
                                        (new_pass_id, name, age, gender, address, contact))
                            mydb.commit()
    ...
    with loginsection:
        with st.form('login_form'):
            ...
            if st.form_submit_button('Login'):
                ...
                elif username in login_df['UserName'].values and password == \
                        login_df['Password'][login_df['UserName'] == username].values[0]:
...
def show_admin_page():
    ...
    page = st.sidebar.radio('', (
    'Show Trains', 'Show Users', 'Add Train', 'Edit Train', 'Remove Train', 'Remove User', 'Logout'))

    if page == 'Show Trains':
        st.subheader('Trains')
        train_df2 = train_df.rename(
            columns={'Tr Number': 'Train Number', 'Tr Name': 'Train Name', 'Tr From': 'From', 'Tr To': 'To',
                     'Tr Dept Time': 'Departure', 'Tr Arr Time': 'Arrival', 'Tr Total Seats': 'Total Seats',
                     'Tr Avl Seats': 'Available Seats', 'Tr Price': 'Price'})
        st.table(train_df2)

    elif page == 'Show Users':
        st.subheader('Users')
        pass_df['Pass Contacts'] = pass_df['Pass Contacts'].astype('int64')
        pass_df2 = pass_df.rename(
            columns={'Pass ID': 'Passenger ID', 'Pass Name': 'Name', 'Pass Age': 'Age', 'Pass Gender': 'Gender',
                     'Pass Address': 'Address', 'Pass Contacts': 'Contact'})
        st.table(pass_df2)
        ...
        if st.button('Add Train'):
            ...
            if train_number != '' and len(
                    train_number) == 5 and train_name != '' and train_source != '' and train_destination != '' and train_source_time != '' and train_destination_time != '' and train_total_seats != '' and train_fare != '':
                try:
                    int(train_number)
                    int(train_total_seats)
                    int(train_fare)
                    # insert into mysql
                    sql.execute('insert into train values (%s,%s,%s,%s,%s,%s,%s,%s,%s)', (
                    train_number, train_name, train_source, train_destination, train_source_time,
                    train_destination_time, train_total_seats, train_available_seats, train_fare))
                    mydb.commit()
        ...
        if st.button('Edit Train'):
            if train_number != '' and len(
                ...
                try:
                    ...
                    # insert into mysql
                    sql.execute(
                        'update train set `Tr Name` = %s, `Tr From` = %s, `Tr To` = %s, `Tr Dept Time` = %s, `Tr Arr Time` = %s, `Tr Total Seats` = %s, `Tr Avl Seats` = %s, `Tr Price` = %s where `Tr Number` = %s',
                        (train_name, train_source, train_destination, train_source_time, train_destination_time,
                         train_total_seats, train_available_seats, train_fare, train_number))
                    mydb.commit()
        ...
        try:
            ...
            train_details = train_details.rename(
                columns={'Tr Number': 'Train Number', 'Tr Name': 'Train Name', 'Tr From': 'From', 'Tr To': 'To',
                         'Tr Dept Time': 'Departure', 'Tr Arr Time': 'Arrival', 'Tr Total Seats': 'Total Seats',
                         'Tr Avl Seats': 'Available Seats', 'Tr Price': 'Price'})
        ...
        try:
            ...
            pass_details = pass_details.rename(
                columns={'Pass ID': 'Passenger ID', 'Pass Name': 'Name', 'Pass Age': 'Age', 'Pass Gender': 'Gender',
                         'Pass Address': 'Address', 'Pass Contacts': 'Contact'})
